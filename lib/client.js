window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function scheduleAutoReload(setState, setError) {
      ...
      const poll = (oldBoot) => {
        ...
        getBoot().then((boot) => {
          ...
          setTimeout(() => poll(oldBoot), 1200)
        }).catch(() => setTimeout(() => poll(oldBoot), 1200))
      }
      // Capture the pre-restart marker while the old service is still alive,
      // then start polling after a short grace period (the detached restart
      // script kills the old process ~2 s after the trigger response).
      getBoot().then((oldBoot) => {
        setTimeout(() => poll(oldBoot), 3000)
      }).catch(() => {
        setTimeout(() => poll(null), 3000)
      })
    }
          ...
          fetch(RESTART_PATH, { method: "POST" })
            ...
            .then((body) => {
              if (body && body.ok === false) {
                ...
                scheduleAutoReload(setState, setError)
              }
            })
            .catch((err) => {
              setState("idle")
