window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const HEALTH_PATH = "/api/restart-service-button/health"
    ...
    // Read the current service boot marker ("" when unavailable).
    function getBoot() {
      return fetch(HEALTH_PATH, { method: "GET", cache: "no-store" })
        .then((res) => (res.ok ? res.json() : Promise.reject(new Error("health " + res.status))))
        .then((body) => (body && body.boot ? body.boot : ""))
    }

    // After a successful restart trigger: wait for the old process to die,
    // poll until the NEW process answers with a different boot marker, then
    // auto-reload the page. Gives up after 90 s and returns to idle.
    // `oldBoot` is the boot marker captured BEFORE the trigger; null means
    // "no baseline" — reload on the first healthy response.
    function scheduleAutoReload(setState, setError, oldBoot) {
      const started = Date.now()
      const poll = () => {
        if (Date.now() - started > 90000) {
          setState("idle")
          setError("重启超时：服务未在 90 秒内恢复，请手动刷新页面")
          return
        }
        getBoot().then((boot) => {
          const isNew = oldBoot === null ? true : boot !== oldBoot
          if (boot !== "" && isNew) {
            console.log("[restart-service-button] new service detected, reloading")
      ...
      // Grace: the detached restart script kills the old process ~2 s after
      // the trigger, so wait before the first poll to avoid hitting it.
    ...
    function RestartButton() {
      ...
      const onClick = () => {
        if (state === "idle") {
          ...
          setState("busy")
          setError("")
          // Capture the pre-restart boot marker first so we can recognize the
          // new process even if the trigger response is cut short by the kill.
          getBoot().then((bootBefore) => {
            fetch(RESTART_PATH, { method: "POST" })
              ...
              .then((body) => {
                if (body && body.ok === false) {
                  ...
                  scheduleAutoReload(setState, setError, bootBefore)
                }
              })
              .catch(() => scheduleAutoReload(setState, setError, bootBefore))
          }).catch(() => {
            // No baseline (service already dying); trigger anyway and reload
            ...
            fetch(RESTART_PATH, { method: "POST" })
              ...
              .then((body) => {
                if (body && body.ok === false) {
                  ...
                  scheduleAutoReload(setState, setError, null)
                }
              })
              .catch(() => scheduleAutoReload(setState, setError, null))
          })
        }
      }

      const text = state === "idle" ? "重启服务" : state === "confirm" ? "确认重启？" : "重启中…"
      const title = error ? error : "一键重启 DSH 后端服务（dsh web）"
