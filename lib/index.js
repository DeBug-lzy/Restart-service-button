...
export function apply(ctx) {
  ...
  // Boot marker: unique per process, so the browser can tell a fresh
  // (restarted) service apart from the old one it triggered the restart from.
  const bootId = String(process.pid) + '-' + Date.now()
  ...
  // Loopback-only health probe used by the button to auto-reload once the
  // restarted service is back (the boot marker identifies the new process).
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: HEALTH_PATH,
    handler: async (req, res) => {
      ...
      writeJson(res, 200, { ok: true, boot: bootId })
    },
  }), 'restart-service-button: health route')
}
